...
String _buildRows(List<dynamic> items, {String? paidAmountClass}) {
  if (items.isEmpty) {
    return '<tr><td colspan="4">No entries</td></tr>';
  }

  return items.map((raw) {
    final item = raw as Map<String, dynamic>;
    final paidClass = paidAmountClass ?? '';
    return '''
      <tr>
        ...
        <td class="$paidClass">${_escapeHtml(_money(item['paidAmount'] as Map<String, dynamic>?))}</td>
      </tr>
    ''';
  }).join();
}
...
<html>
  <head>
    ...
    <style>
      ...
      .text-success { color: var(--mvcs-green); font-weight: bold; }
      .text-danger { color: var(--mvcs-red); font-weight: bold; }
      .section-title {
        font-size: 16px;
        font-weight: 700;
        margin: 0 0 12px 0;
  ...
  <body>
    ...
    <table>
      ...
      <tbody>${_buildRows(paidMembers, paidAmountClass: 'text-success')}</tbody>
    </table>
    ...
    <table>
      ...
      <tbody>${_buildRows(peopleToPay, paidAmountClass: 'text-danger')}</tbody>
    </table>
    <footer>Generated by MVCS, a product of Masaka volleyball club</footer>
    </div>
    </div>
